...
class WaitingService extends BaseService {
  async find(
    page: number = 1,
    pageSize: number = 10,
  ): Promise<{data: Waiting[]; count: number}> {
    let params: any = {
      per_page: pageSize,
      page: page,
    };
    ...
    try {
      ...
      if (error.response) {
        ...
        throw new Error(
          'No pudimos recuperar tu lista de espera, por favor intenta de nuevo en unos momentos.',
        );
      }
    }
  }
  ...
  async create(lesson_id: number | string): Promise<Waiting | undefined> {
    try {
      ...
      if (error.response) {
        ...
        throw new Error(
          'Ocurrió un error al anotarte a la lista de espera, intenta de nuevo.',
        );
      }
    }
  }
