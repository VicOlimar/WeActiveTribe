  ...
  async find(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      const query: FindAndCountOptions = {
        limit,
        offset,
        order: [[{ model: Lesson, as: 'lesson' }, 'starts_at', 'DESC']],
        include: [
          {
            model: Place,
            attributes: ['location'],
            include: [{ model: Studio, attributes: ['name'] }],
          },
        ],
        attributes: [
          'id',
      ...
      if (user.role === 'user') {
        ...
        query.include.push({
          model: Lesson,
          include: [{ model: Studio }, { model: Instructor }],
        });
    ...
    try {
      ...
      if (!user) {
        return Controller.badRequest(
          res,
          'El usuario indicado no fué encontrado.',
        );
      }
      ...
      if (req.session.user.role !== 'admin') {
        if (
          moment()
            .utcOffset('-05:00')
            .diff(moment.tz(lesson.ends_at, 'America/Merida'), 'minutes') > 0
        ) {
          return Controller.badRequest(
            res,
            'No puedes reservar la clase, la clase ha iniciado o ha terminado.',
          );
        }
      }
      ...
      if (req.session.user.role !== 'admin') {
        const diffDays = moment(lesson.starts_at)
          .utc()
          .utcOffset('-05:00')
          .startOf('day')
          .diff(
        ...
        if (diffDays >= 7) {
          ...
          return Controller.badRequest(
            res,
            `No puedes reservar esta clase aún, intenta el día ${
              diffDays - 7 === 0 ? 'de mañana.' : `${futureDay} del mes.`
            }`,
          );
        }
      }
      ...
      // Sending email
      const date = moment(lesson.starts_at)
        .utc()
        .utcOffset('-05:00');
    ...
    try {
      ...
      if (
        ...
        moment(lesson.starts_at)
          .utc()
          .utcOffset('-05:00')
          .diff(moment().utcOffset('-05:00'), 'hours') < limitHours
      ) {
        return Controller.badRequest(
          res,
          `No es posible cancelar una reservación ${limitHours} horas antes de la clase.`,
        );
      }
      ...
      try {
        const query: FindOptions = {
          ...
          include: [
            { model: Lesson },
            { model: User, as: 'user' },
            { model: User, as: 'canceled_by_user' },
          ],
        };
