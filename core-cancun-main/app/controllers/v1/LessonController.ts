...
export class LessonController extends Controller {
    ...
    try {
      ...
      if (req.session.logged && req.session.user.role === 'admin') {
        ...
        query.where = {
          // @ts-ignore
          ends_at: {
            [Op.between]: [startOfWeek.toISOString(), endOfWeek.toISOString()],
          },
        };
        query.include = [
          {
      ...
      if (studio) {
        lessons = (await studio.$get('lessons', query)) as Lesson[];
      ...
      const lessonsResult: any = lessons.map(async lesson => {
        ...
        const locked = await BlockedPlace.count({
  ...
  async findOne(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      const instructors = lesson.instructors.map(instructor =>
  ...
  async assistants(
    ...
    try {
      ...
      const reserves = await Reserve.findAll({
        ...
        include: [{ model: User, as: 'user', include: [Profile] }, Place],
      });
  ...
  async waitingList(
    ...
    try {
      ...
      const waitings = await Waiting.findAll({
        where: { lesson_id: lesson.id, canceled: false },
        include: [
          { model: User, as: 'user', include: [Profile] },
          { model: Reserve, as: 'reserve' },
        ],
      });

      return Controller.ok(res, null, waitings);
    } catch (err) {
      return next(err);
    }
  }
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      */
      ...
      const lessonInstructors = await LessonInstructors.findAll({
        where: { lesson_id: lesson.id },
      });

      await lessonInstructors.map(lessonInstructor =>
        lessonInstructor.destroy(),
      );
      ...
      const newDate = moment(req.body.starts_at).utcOffset('-05:00');

      const instructorsNameArray: string[] = await Promise.all(
        req.body.instructors.map(async id => {
