...
export class UserController extends Controller {
  ...
  async findMe(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      const available_data = await user.$get('available_credits', {
        include: [LessonType, Studio],
      });
      const available_online = await user.$count('available_online_credits');
      const available_online_data = await user.$get(
      ...
      return Controller.ok(res, null, {
        ...
        profile,
        credits,
        reserves: { next_reserves: reserves.count },
      });
    } catch (error) {
      return next(error);
    }
      ...
      if (req.body.profile) {
        ...
        if (req.body.profile.emergency_contact_name) {
          profile.emergency_contact_name =
            req.body.profile.emergency_contact_name;
        }
  ...
  async pauseValidity(req: Request, res: Response, next: NextFunction) {
    try {
      const user: User = req.session.user;
      if (
        user.available_credits.length > 0 ||
        user.available_online_credits.length > 0
      ) {
        await pausedValidity(
      ...
      return Controller.ok(
        res,
        'El usuario no tiene credtiros para pausar',
        null,
      );
    } catch (error) {
      return next(error);
  ...
  async reactivateValidity(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return Controller.ok(
        res,
        'El usuairo no tiene creditos para reactivar',
        null,
      );
    } catch (error) {
      return next(error);
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        const user = await User.findByPk(req.params.id, {
          ...
          include: [
            { model: Credit, as: 'available_credits', include: [LessonType, Studio] },
            { model: OnlineCredit, as: 'available_online_credits' },
          ],
        });
        if (!user) {
