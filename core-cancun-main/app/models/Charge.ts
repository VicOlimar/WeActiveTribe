...
export class Charge extends Model<Charge> {
  ...
  payment_method: 'conekta' | 'paypal' | 'cash' | 'credit-card' | 'terminal' | 'stripe';
  ...
  static async set(charge: Charge, options: any) {
    if (!isNullOrUndefined(charge.expires_at)) {
      return charge;
    }
  ...
  static async createCredits(charge: Charge, options: any) {
    ...
    if (options == 'courtesy') {
      if (charge.status === 'paid') {
        let CreditClass = charge.plan_name.includes('online')
          ? OnlineCredit
          : Credit;
        const date = charge.expires_at;
        const credits = Array(+charge.total_credits)
          .fill(date)
          .map(async date => {
      ...
      if (charge.status === 'paid') {
        const plan = await Plan.findOne({ where: { name: charge.plan_name } });
        let CreditClass =
          plan.credit_type === 'classic' ? Credit : OnlineCredit;
  ...
  static async verifyCancel(charge: Charge, options: any) {
    if (charge.status === 'canceled') {
      ...
      if (credits > 0) {
        throw new Error(
          'No es posible cancelar un cargo con créditos utilizados.',
        );
      }
    }
