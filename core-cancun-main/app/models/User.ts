...
export class User extends Model<User> {
  ...
  async createCustomer(): Promise<string> {
    ...
    const { PaymentGateway, paymentGatewayName } = await PaymentGatewayFactory.getGateway();
    const client_id = await PaymentGateway.createCustomer(this);

    await PaymentCustomer.create({
      payment_key: client_id,
      payment_gateway: paymentGatewayName.toLowerCase(),
  ...
  async mailchimpRegister(): Promise<string> {
    try {
      const response = await mailchimp.post(
        `/lists/${config.mailchimp.list_id}/members`,
        {
  ...
  async updateCustomer(): Promise<string> {
    ...
    const customer = await PaymentGateway.updateCustomer(this);
    return customer.id;
  }

  async getCustomer(plain = false): Promise<any> {
    try {
      const { PaymentGateway, paymentGatewayName } = await PaymentGatewayFactory.getGateway();
      const profile = (await this.$get('profile')) as Profile;
      ...
      const customer = await PaymentGateway.findCustomer(this);
      if (plain) {
        return 'toObject' in customer ? customer.toObject() : customer;
      } else {
        return customer;
      }
    } catch (err) {
      await this.createCustomer();
      return this.getCustomer(plain);
    }
  }
  ...
  async sendPurchaseEmail(plan: Plan): Promise<any> {
  ...
  async addCredits(charge: Charge): Promise<any> {
    if (charge.status === 'paid') {
      ...
      const credits = Array(plan.credits)
        .fill(date)
        .map(async date => {
  ...
  async sendPurchaseFailureEmail(charge: Charge): Promise<any> {
  ...
  async sendPushNotification(title: string, subtitle: string, content: string) {
    const notification = await axios.post<OneSignalResponse>(
      `${config.onesignal.api_url}`,
      {
