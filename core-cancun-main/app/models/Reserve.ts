...
export class Reserve extends Model<Reserve> {
  ...
  static async checkIfAvailable(reserve: Reserve, options: any) {
    ...
    const lesson = await Lesson.findByPk(reserve.lesson_id, {
      include: [Studio, LessonType],
    });
    const is_online = lesson.studio.slug === 'online';

    if (!lesson.community) {
      if (lesson.lesson_type) {
        await validateCreditsType(
          lesson.lesson_type_id,
          reserve.user_id,
          defaultCreditsByClass,
          is_online,
        );
      }

      if (lesson.studio_id && !lesson.lesson_type) {
        await validateStudioCredits(
          lesson.studio_id,
          reserve.user_id,
          defaultCreditsByClass,
        );
      ...
      const credit = await useACredit(
        reserve.user_id,
        is_online ? 'online' : 'classic',
        lesson.lesson_type_id,
        lesson.studio_id,
      );
      if (credit) {
  ...
  static async checkWaitingList(reserve: Reserve, options: any) {
    const lesson = (await reserve.$get('lesson', {
    ...
    if (
      now.hours() >= max_time_hour_to_cancel &&
      hours_difference <= max_time_difference
    ) {
      if (lesson_date.hours() <= 10) {
        // 10 AM
        return;
      }
    }
    ...
    if (waitings.length) {
      ...
      while (available) {
        try {
          ...
          const instructorsNameArray: string[] = await Promise.all(
            lesson.instructors.map(async instructor => {
          ...
          const date = moment(waiting.lesson.starts_at).utcOffset('-05:00');
          try {
            ...
            log.info(
              `Email ${waiting.user.email} de envio de waiting list fallido.`,
            );
          }
          ...
          try {
            ...
            if (!phone) {
              throw new Error(
                'El usuario no cuenta con un número de teléfono.',
              );
            }
            const msg = `WE: Tienes el lugar ${
              place.location
            } de la clase de ${date.format(
              config.date_format.time,
            )} el día ${date.format(
            ...
            log.info(
              `SMS ${waiting.user.email} de envio de waiting list fallido.`,
            );
          }
        } catch (err) {
  ...
  static async updateCredit(reserve: Reserve) {
    ...
    if (!lesson.community) {
      ...
      if (lesson.studio.slug === 'online') {
        ...
        credit = await OnlineCredit.findOne({
          where: { id: reserve.online_credit_id },
        });
      } else {
        console.log(lesson.studio.slug);
