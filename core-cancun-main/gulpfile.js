...
gulp.task(
  'clean-serve',
  gulp.series('clean', 'copy-views', 'copy-locales', 'serve'),
);

gulp.task(
  'build',
  gulp.series('clean', 'compile', 'copy-views', 'copy-locales'),
);

// first time cleans and compiles, subsecuent times only compiles
gulp.task(
  'watch',
  gulp.series('clean-serve', () => {
    gulp.watch('app/**/*.ts', gulp.series('serve'));
  }),
);

gulp.task('test', gulp.series('build', shell.task('npm test')));
