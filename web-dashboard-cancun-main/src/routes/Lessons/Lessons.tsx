...
import { Studio as StudioClass } from './../../api/Studio/Studio';
import './Lessons.scss';
...
class Lessons extends Component<Props, State> {
  constructor(props: Props) {
    ...
    this.state = {
      ...
      currentDate: moment().utcOffset('-05:00').toDate(),
  ...
  handleLessonClick = (lesson: Lesson) => {
    ...
    if (lesson.available === 0 && studio) {
      this.props.history.push(
        `/studio/${studio.slug}/lesson/${lesson!.id}`,
        this.state.currentDate ? this.state.currentDate : null,
      );
    } else if (studio) {
      if (!previewNextWeek) {
        this.props.history.push(
  ...
  filterCalendar = (instructor: Instructor) => {
    const selectedInstructor: Instructor | undefined =
  ...
  loadWeek = async (date: Date) => {
    try {
      ...
      if (studio) {
        ...
        let end_date = moment(date).add(6, 'days').endOf('day');
  ...
  findLessons = async (studio: StudioClass) => {
    try {
      ...
      this.setState({
        error: true,
        message: 'Error cargando las clases.',
        alertVariant: '',
        loading: false,
      });
    }
