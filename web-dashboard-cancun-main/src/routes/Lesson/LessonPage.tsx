...
import { WaitingList } from './components/WaitingList/WatingList';
...
class LessonPage extends Component<Props, State> {
  ...
  /**
   * Get waitings of a lesson
   */
  getWaitings = async () => {
    const { params } = this.props.match;
    if (params.slug && params.lesson) {
      const waitings = await LessonService.getWaitingAssistants(
        params.slug,
        params.lesson,
      );
      if (waitings) {
        await this.setState({ waitings });
      }
    } else {
      this.setState({
        error: true,
  ...
  getPageTitle = () => {
    ...
    const starts_at = lesson
      ? moment(lesson!.starts_at).utcOffset('-05:00').format(this.DATE_FORMAT)
      : '';
        ...
        >
          ...
          <div className="inline-flex lesson_section__indications">
            <div className="lesson_section__indications__first">
              <div
                className={`lesson_section__indications__first__icon lesson_section__indications__first__icon-${studio ? studio!.slug : ''}`}
              ></div>
              <span>Reservado</span>
            </div>
