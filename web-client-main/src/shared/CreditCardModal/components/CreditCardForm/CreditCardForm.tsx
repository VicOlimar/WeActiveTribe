...
const CreditCardForm = (props: Props) => {
  ...
  const { loading, showCardSelector } = props;
  ...
  const creditCardValidationSchema = Yup.object().shape({
    ...
    card_one: Yup.string().when('card', getSchema),
    card_two: Yup.string().when('card', getSchema),
    card_three: Yup.string().when('card', getSchema),
    card_four: Yup.string().when('card', getSchema),
    month: Yup.string().when('card', getSchema),
    year: Yup.string().when('card', getSchema),
    cvc: Yup.string().when('card', getSchema),
    isDefault: Yup.bool(),
    code: Yup.string(),
  });
    ...
    >
      {formProps => (
        <Form className={'credit_card__form'}>
          ...
          <Row>
            {
              showCardSelector && !props.paypalOnly && !props.loading && <Col xs={12}>
                <Select
                  label={''}
                  name={'card'}
          ...
          {
            formProps.values.card === '' && !props.paypalOnly && !props.loading && <Fragment>
              <Row>
                <Col xs={12}>
                  <Input
                    label={''}
          ...
          <Row>
            <Col xs={6}>
              {
                formProps.values.card === '' && !props.paypalOnly && cards.length !== 0 && !props.loading && <div className={'credit_card__form__checkbox'}>
                  <Input
          ...
          {
            !props.loading && props.showDiscount && < Row >
            <Col xs={4} lg={4}>
              <Input
                label={''}
                type="text"
      ...
      <Row>
        {
          props.paypalOnly && <Col xs={12} lg={props.paypalOnly ? 12 : 6}>
        ...
        {
          !props.paypalOnly && !props.loading && <Col xs={12} lg={6}>
            <Button type='submit' className='credit_card__form__buy' text={props.confirmButtonText} loading={loading} />
          </Col>
        }
        {
          props.loading && <CircularLoader message={props.loadingMessage ? props.loadingMessage : 'Procesando tu pago...'} />
        }
      </Row>
        </Form>
  )
