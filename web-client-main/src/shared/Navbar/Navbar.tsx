...
import LogoMid from "./assets/we-mid.png";
...
class AppNavbar extends Component<Props, State> {
  ...
  displayStudios = () => {
    const { showOnlyOne } = this.state;
    const updateStudios = this.props.studios.filter(
      (studio: Studio) => studio.slug !== "online"
    );
    return updateStudios.map((studio: Studio, index: number) => (
      <Link
        key={index}
        className={`nav-item nav-link studio-name app-nav__studio__name app-nav__studio__logo ${
          !isUndefined(showOnlyOne)
            ? `app-nav__studio__logo-${studio.slug}${
                this.isActiveStudio(studio.slug) ? "-active" : ""
              }`
            : ""
        }`}
      ...
      >
        {studio.slug === "we-ride" ? (
          <img
            className={`app-nav__studio__logo`}
            src={WeRide}
  ...
  goToPlans = () => {
    if (this.props.history.location.pathname !== "/merida") {
      window.location.href = "/merida?plans=true";
    } else {
  ...
  render() {
    ...
    const isWeMid = this.props.history.location.pathname !== "/";
    return (
      <div>
        <LoginModal
          show={showModal}
          ...
          notifyError={(error: any) =>
            this.setState({ message: error, showMessage: true })
          }
        />
        <Navbar
          expand="lg"
          ...
          onToggle={() => this.handleNavbarExpanded()}
        >
          <Navbar.Brand>
            <Link className="navbar-brand" to="/">
              <img
                className={isWeMid ? "app-nav__logo-mid" : "app-nav__logo"}
                src={isWeMid ? LogoMid : Logo}
                alt="App logo"
              />
            </Link>
          </Navbar.Brand>
          <Navbar.Toggle
            ...
            children={<img className="app-nav__option" src={Bars} alt="bars" />}
            onClick={() => {
              this.handleNavbarExpanded();
            }}
          />
          <Navbar.Collapse className="text-center" id="basic-navbar-nav">
            <Nav>{showStudios && this.displayStudios()}</Nav>
            <Nav className="ml-auto">
              <Nav.Item className="app-nav__item" onClick={this.goToAbout}>
                <img
                  className="app-nav__option"
                  src={WeActive}
                  alt="we active"
                />{" "}
                Nosotros
              ...
              {user && (
                <Nav.Item className="app-nav__item" onClick={this.goToCards}>
                  <img
                    className="app-nav__option"
                    src={CreditCard}
                    alt="credit-card"
              ...
              <Nav.Item
                ...
                onClick={
                  user ? () => this.goToProfile(true) : this.handleModalOpen
                }
              >
                <img
                  className="app-nav__option"
              ...
              {user && (
                <Nav.Item className="app-nav__item" onClick={this.doLogout}>
                  <img
                    className="app-nav__option"
                    src={Logout}
                    alt="credit-card"
