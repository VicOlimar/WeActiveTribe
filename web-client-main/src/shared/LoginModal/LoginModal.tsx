...
class LoginModal extends Component<Props, State> {
  ...
  onSubmit = async (values: any) => {
    ...
    if (form === FORMS.login) {
      ...
      response = await AuthService.register(values.email, values.password, values.name, values.last_name, values.phone);
      if (response !== null) {
  ...
  switch = () => {
    const { form, switching } = this.state;
    setTimeout(() => {
      this.setState({ switching: !switching, height: form === FORMS.register ? this.register_modal_height : this.login_modal_height });
    }, this.animation_time);
  }
    ...
    return (
      <div className={'login__modal'}>
        <Modal
          ...
          footer={<Footer text={form === FORMS.register ? 'Iniciar sesión' : 'Registrarse'} onClick={this.switchForms} />}
          onClose={() => this.props.onClose(this.switch)}
        >
          <div className={`login__modal__form ${switching ? 'hide' : 'show'}`}
            style={{ height }}>
            {
              !switching && (form === FORMS.register ? <RegisterForm onSubmit={this.onSubmit} loading={loading} /> : <LoginForm onClose={this.props.onClose} onSubmit={this.onSubmit} loading={loading} />)
            }
          </div>
        </Modal>
      </div>
