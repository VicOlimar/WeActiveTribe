...
class LessonPage extends Component<Props, State> {
  ...
  requestCredits = async (place: Place) => {
    ...
    if (me && lesson) {
      ...
      } if (me.user.pause === true) {
        this.setState({ message: 'No puedes reservar ya que tu suscripción se encuentra pausada.', showMessage: true });
      } else {
  ...
  validatePlanPurcharseResponse = (plan: Plan, response: PlanPurcharseResponse | string) => {
    if (typeof response === 'string') {
      ...
      this.setState({ errorMessage: undefined, message: `Se han agregado ${plan.credits} ${plan.credits > 1 ? 'clases' : 'clase'} a tu cuenta`, showMessage: true });
    }
  }
  ...
  canSeeNextOption = () => {
    ...
    if (!isUndefined(lesson)) {
      const lesson_starts = moment(lesson!.starts_at).utcOffset("-06:00");
        ...
        <AppHeader
          ...
          leftComponent={
            <LessonHeader
              ...
              leftText={lesson ? moment(lesson!.starts_at).utcOffset("-06:00").format(this.DATE_FORMAT) : ''}
        ...
        </div>
        {
          showBuyCredits && !showSpecialMessage && !showHiitBuddiesMessage && !showRideAndAbsMessage ? <Fragment>
            ...
            subtitle={<p>¡50 minutos de Cardio Intenso!<br />
              25 minutos Weride + 25 minutos Wetrain</p>}
            steps={[ 
              'Reserva tu lugar en Wetrain o Weride',
              'Si reservas primero en Wetrain. Elige tu bici una vez que llegues al estudio y firmes para tu clase de Wetrain. Te entregaremos tus zapatos de bici en ese momento.',
              `Tu clase inicia a las ${lesson ? moment(lesson!.starts_at).utcOffset("-06:00").format('h:m A') : ''} en el estudio que reservaste.`
            ]}
          ...
          /> : showRideAndAbsMessage && !showSponsoredMessage ? <SpecialMessage
            ...
            showClose={true}
            title='RIDES + ABS'
            subtitle={<p>¡45 minutos de indoor cycling + 15 minutos de ejercicios de abdomen en el estudio de Wetrain!</p>}
            steps={[]}
          /> : showDescription &&
            !showSpecialMessage &&
            !showSponsoredMessage &&
          ...
          /> :
            <div className='lesson_section__content'>
              <MessageBackDrop
                ...
                place={placeSelected && placeSelected!.location}
