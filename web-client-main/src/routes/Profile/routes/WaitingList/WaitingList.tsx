...
class WaitingList extends Component<Props, State> {
  ...
  mapWaitingData = (waitingList: Waiting[]) => {
    ...
    const mappedWaiting = waitingList.map(waiting => {
      return {
        label: waiting!.lesson ? moment(waiting!.lesson.starts_at).utcOffset("-06:00").format(this.DATE_FORMAT) : 'No disponible',
    ...
    if (isEditing) {
      return <span>
        ...
        <span className={`waitings__studio waitings__studio-${waiting.lesson.studio.slug}`}>
          <img src={waiting.lesson.studio.slug === 'we-hiit' ? WE_TRAIN : waiting.lesson.studio.slug === 'online' ? WE_ONLINE : WE_RIDE} alt='studio logo' />
        </span>
