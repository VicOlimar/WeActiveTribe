...
class Reservations extends Component<Props, State> {
  ...
  mapReservationsData = (reservations: Reservation[]) => {
    ...
    const mappedReservations = reservations.map(reservation => {
      return {
        label: reservation!.lesson ? moment(reservation!.lesson.starts_at).utcOffset("-06:00").format(this.DATE_FORMAT) : 'No disponible',
    ...
    if (isEditing) {
      return <span>
        ...
        <span className={`reservations__studio reservations__studio-${reservation.lesson.studio.slug}`}>
          <img src={reservation.lesson.studio.slug === 'we-hiit' ? WE_TRAIN : reservation.lesson.studio.slug === 'online' ? WE_ONLINE : WE_RIDE} alt='studio logo' />
        </span>
      ...
      let placeEnd = 0;
      if (reservation.place.location === '7B') {
        placeEnd = 8
      }else if (reservation.place.location.indexOf('A') === 1 || reservation.place.location.indexOf('A') === 2 || reservation.place.location.indexOf('B') === 1 || reservation.place.location.indexOf('B') === 2) {
        let p = parseInt(reservation.place.location.replace('A','').replace('B',''))
        if (p >= 8) 
          placeEnd = p + 1
        else
          placeEnd = p
      ...
      return reservation.lesson ? <Fragment>
        ...
        <span className={`reservations__instructor`}><span className={`${reservation.canceled ? 'reservations__decoration' : ''}`}>(Lugar {placeEnd === 0 ? reservation.place.location : placeEnd}) {instructorsNames.length > 1 ? instructorsNames.join(' + ') : instructorsNames[0]}</span> {reservation.canceled ? <span className='reservations__canceled'>Cancelada</span> : reservation.lesson.studio.slug === 'online' && reservation.lesson.meeting_url ? <span className='reservations__see_lesson' onClick={(event: any) => this.showMeeting(reservation.lesson.meeting_url)}>Ver clase</span> : ''}</span>
      </Fragment> : ''
    }
  }
