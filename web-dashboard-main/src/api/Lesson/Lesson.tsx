...
class LessonService extends BaseService {
    ...
    async create(
        ...
        try {
            ...
            if (error.response.data.message === 'hours_overlap') return 'hours_overlap'
            else if (typeof error.response.data.data[0] === 'string') return error.response.data.data[0];
    ...
    async findOne(studio: Studio, id: number): Promise<Lesson | undefined> {
        try {
            const response = await axios.get<Response<Lesson>>(`${this.url}/studio/${studio.slug}/lesson/${id}`, this.getHeaders());
            const { data: axiosData } = response;
            return axiosData.data as Lesson;
        } catch (error) {
            return undefined;
    ...
    async getCanceledAssistants(slug: string, id: number): Promise<any | undefined> {
        try {
            const response = await axios.get<Response<any>>(
