    ...
    try {
      ...
      if (opMethod === "courtesy") {
        if (credits) {
          ...
          throw new Error(
            "No se puede crear una cortesía sin asignar créditos"
          );
      } else {
      ...
      if (response) {
        ...
        throw new Error(
          "Ocurrió un error al asignar los créditos, prueba de nuevo"
        );
      }
    } catch (error) {
      props.parentLevelAlert(true, (error as Error).message, "error");
    }
    callback && callback();
  };
  ...
  return (
    <React.Fragment>
      <Modal show={props.show} onHide={props.parentLevelClose}>
        <Modal.Header>
          <Modal.Title>Agregar cargo</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <ChargeForm
            show={props.show}
