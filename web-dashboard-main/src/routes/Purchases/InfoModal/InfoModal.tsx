...
class InfoModal extends Component<Props, State> {
    ...
    try {
        if (this.props.id) {
          ...
          if (response) {
            ...
            if (user!.user) {
              this.setState({ purchase: response.data, username: `${user!.user.name} ${user!.user.last_name}` });
            } else {
  ...
  render() {
    ...
    return ( 
      <Modal show={this.props.show} onEnter={this.getPurchase} onHide={this.props.parentHandleClose} onExited={this.resetState}>
        <Modal.Header>
