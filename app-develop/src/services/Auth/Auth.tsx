...
export class AuthService extends BaseService {
  ...
  async login(email: string, password: string): Promise<any | string> {
    try {
      ...
      const response = await Axios.post<AuthResponse>(`${this.url}/auth/login`, body);
  ...
  async register(email: string, password: string, name: string, last_name: string | null = null) {
    try {
      const body = {
        email,
        password,
      ...
      const response = await Axios.post<AuthResponse>(`${this.url}/auth/register`, body);
      return response.data.data;
    } catch (error) {
  ...
  async refreshToken(token: string): Promise<any | string> {
    try {
      const header = await this.getHeaders();
      const response = await Axios.post<AuthResponse>(`${this.url}/auth/refresh`, {}, header);
      const { data: axiosData } = response;
  ...
  async changePassword(oldPass: string, newPass: string): Promise<any | string> {
    try {
      const header = await this.getHeaders();
      const response = await Axios.post<any>(
      ...
      if (error.response) {
        throw new Error(error.response.data.message === 'unauthorized' ? defaultMessage : error.response.data.message);
      } else {
