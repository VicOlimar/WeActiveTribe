...
import { Order, OrOperator, Op, FindAndCountOptions, WhereOptions } from 'sequelize';
...
export async function validateCreditsType(lesson_type_id: number, user_id: number, credits_number: number, isOnline: boolean) {
  ...
  if (credits_number > credits.length) {
    throw new Error(`No tienes créditos suficientes para este tipo de clase. Compra un paquete para la clase ${lessonType.name}`);
  }
}

export async function validateStudioCredits(studio_id: number, user_id: number, credits_number: number) {
  const user = await User.findByPk(user_id);
  ...
  if ((credits_number > studioCredits) && (credits_number > normalCredits)) {
    throw new Error(`No tienes créditos suficientes para esta clase. Compra un paquete de clases o un paquete para el estudio ${studio.name}`);
  }
}
...
export async function useACredit(user_id: number, type = 'classic', lesson_type_id: number = null, studio_id: number = null): Promise<Credit | OnlineCredit> {
  const user = await User.findByPk(user_id);
  let credits: Credit[] | OnlineCredit[];
  let studioCredits: Credit[] = [];
  let order: Order = [['expires_at', 'ASC']];

  let whereOptions: WhereOptions = lesson_type_id !== null ? {
    lesson_type_id,
    studio_id: null,
  } : {
    studio_id: null,
  ...
  studioCredits = (await user.$get('available_credits', studioCreditsQuery)) as Credit[];

  if (type === 'classic') {
    credits = (await user.$get('available_credits', query)) as Credit[];
  } else {
    credits = (await user.$get('available_online_credits', query)) as OnlineCredit[];
  }
  ...
  let credit = studioCredits.length > 0 ? studioCredits.shift() : credits.shift();
...
export async function pausedValidity(availableCredits: Credit[], availableOnlineCredits: OnlineCredit[]) {
  let credits: Credit[] = availableCredits
  let onlineCredits: OnlineCredit[] = availableOnlineCredits
  const currentdate = moment().utcOffset('-06:00').toDate()
...
export async function reactivatedValidity(userId: number) {
  ...
  if (credits.length > 0 || onlineCredits.length > 0) {
    let date = moment().utcOffset('-06:00').toDate()
    credits.map(async (credit: Credit) => {
...
export async function addCourtesy(
  ...
  charge.payment_type = payment_method;
...
export function compareEqualDateWithoutTime(date1: Date, date2: Date) {
