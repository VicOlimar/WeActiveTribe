...
export class Lesson extends Model<Lesson> {
  ...
  async classReminder() {
    ...
    let time = moment(this.starts_at).utcOffset('-06:00').format('LT');
    ...
    // work around don't repeat notify
    this.send_list_coaches = true;
    await this.save()

    console.log("Lesson reminders: ", this.name, " ID: ", this.id)
  }
