...
export class AuthController extends Controller {
  ...
  private async handleResetChPass(
    ...
    let user = await User.findOne({ where: { id: decodedjwt.id }, include: [Profile] });
  ...
  public async validateJWT(token: string, type: string): Promise<any> {
    ...
    if (decodedjwt.exp <= reqTime && !(daysDifference >= 0 && daysDifference <= config.jwt.max_valid_days_after_exp)) {
  ...
  async login(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (user && (await user.authenticate(password))) {
        ...
        if (req.body.mobile_login && false) {
          const MARCH_MONTH = 2;
          const dateNow = moment().utcOffset('-06:00');
          const PAYMENT_METHOD = 'courtesy';
          const TOTAL_CREDITS_IN_PROMO = 1;
          const SPECIAL_PROMO = 'Cortesía Descarga App';
          const lastCharge = await Charge.findOne({ where: { plan_name: SPECIAL_PROMO, user_id: user.id } });

          if (!lastCharge && dateNow.month() === MARCH_MONTH) {
            const charge = await addCourtesy(user, TOTAL_CREDITS_IN_PROMO, SPECIAL_PROMO, PAYMENT_METHOD);
            credentials = Object.assign(credentials, { charge: charge.get({ plain: true }) });
          }
        }
