...
import { FindAndCountOptions, FindOptions, Op, Includeable, Order } from 'sequelize';
  ...
  async create(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      const schema = yup.object().shape({
        user_id: yup.number().required('No se puede crear un cargo sin usuario'),
        credits: yup.number().required('No se puede crear un cargo de cortesía sin créditos'),
        credit_type: yup.mixed().required('Se debe indicar el tipo de crédito que se dará.').oneOf(['online', 'classic']),
  ...
  async find(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      const query: FindAndCountOptions = {
        order,
        include: [{ model: User }, { model: Credit, as: 'credits' },  { model: OnlineCredit, as: 'online_credits' }],
        where: {}
      };
      ...
      if (req.session.user.role === 'admin') {
        ...
        if (req.query.search || req.query.payment_type) {
          const user_include: Includeable = { model: User, where: { [Op.or]: [] } };
          if (req.query.search) {
            user_include.where[Op.or].push({ email: { [Op.iLike]: `%${req.query.search}%` } });
            user_include.where[Op.or].push({ name: { [Op.iLike]: `%${req.query.search}%` } });
          }

          if (req.query.payment_type) {
            query.where["payment_type"] = or(req.query.payment_type);
          }
        ...
        if (req.query.start_date && req.query.end_date) {
          const start = moment(req.query.start_date).startOf('day').toDate().toISOString();
          const end = moment(req.query.end_date).endOf('day').toDate().toISOString();
      ...
      if (format === 'csv' && req.session.user.role === 'admin') {
        const fields = [
          ...
          { label: 'Método de pago', value: 'payment_type' },
          { label: 'Fecha de pago', value: (row: any) => moment(row.created_at).utcOffset('-06:00').format('D [de] MMMM [-] h:mm A') },
          ...
          {
            label: 'Nombre de Usuario', value: (row: any) => {
              const chargePlain = row.get({ plain: true });
              return chargePlain.user ? `${chargePlain.user.name} ${chargePlain.user.last_name}` : 'No disponible';
            }
          },
