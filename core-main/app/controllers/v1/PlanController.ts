...
export class PlanController extends Controller {
  ...
  static AUTH_TOKEN_BODY = new URLSearchParams([['grant_type', 'client_credentials']]);
  ...
  async find(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      */
      ...
      if (req.session.user && req.session.user.role === "admin" && (!req.query.active_desktop) && (!req.query.active_mobile)) {
  ...
  async findOne(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      return Controller.ok(res, null, req.session.plan);
    } catch (error) {
      return next(error);
    }
  }
  ...
  async create(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      const schema = yup.object().shape({
        ...
        credit_type: yup.mixed().required().oneOf(['classic', 'online']),
  ...
  async purchase(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      if (!plan.active) {
        return Controller.badRequest(res, 'Este plan no se encuentra disponible.');
      }
      ...
      if (!isNullOrUndefined(req.body.card_id)) {
        ...
        const payment_source = payment_sources.find(payment_source => {
          return payment_source.id === req.body.card_id;
        });
        card_id = payment_source ? payment_source.id : req.body.token_id;
      }
      ...
      if (!isNullOrUndefined(req.body.code)) {
        ...
        if (discount.total_uses !== 0) {
          ...
          if (charges.length >= discount.total_uses) {
            discount = null;
            return Controller.badRequest(res, 'El cupón ya ha sido usado las veces permitidas');
          }
        }
        if (discount !== null) {
          // We validate if the discount is a special discount from .env file, we need refactor this if this case repeat in the future
          const specialDiscounts = process.env.SPECIAL_DISCOUNT ? process.env.SPECIAL_DISCOUNT.split(',') : [] || [];
          const specialDiscount = specialDiscounts.find(discount_id => Number(discount_id) === discount.id);

          if (specialDiscount !== undefined && plan.new_users_only) {
            return Controller.badRequest(res, 'El cupón no es válido para planes que son para usuarios nuevos. Intenta seleccionando otro plan.');
          }
        }
      }

      if (plan.new_users_only) {
        const previousCharges = await Charge.count({ where: { plan_name: plan.name, user_id: user.id, status: 'paid' } });
        if (previousCharges > 0) {
          return Controller.badRequest(res, 'El plan es válido unicamente para usuarios nuevos.');
        }
      }

      const charge = await Conekta.createCharge(
        user,
        plan,
        card_id,
        discount,
      ...
      console.log(error);
      return Controller.serverError(res, 'Ocurrió un problema al realizar tu pago, intenta más tarde.', error);
    }
  }
  ...
  async paypal(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      try {
        ...
        paypal_order = await axios.get<any>(`${PlanController.PAYPAL_URL}/v2/checkout/orders/${req.body.order_id}`, {
          headers: {
        ...
        console.log(error);
        return Controller.badRequest(res, 'Ocurrió un error al obtener información de tu método de pago.', null);
      }
      ...
      charge.currency = paypal_order.data.purchase_units[0].amount.currency_code;
  ...
  async pos(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      ...
      charge.auth_code = (req.body.payment_method === 'credit-card') ? req.body.auth_code : '';
      charge.processed_at = new Date();
      charge.paid = total;
      charge.total_credits = plan.credits;

      charge.payment_type = req.body.payment_method;
      ...
      return Controller.serverError(res, 'Ocurrió un problema al asignar los créditos.');
    }
  }
