...
const tasks = {
  ...
  'send-list': async () => {
    ...
    const initial_date = moment().utcOffset('-06:00').format();
    const final_date = moment().utcOffset('-06:00').add('20', 'minutes').format();
    ...
    const classReminder = await Lesson.findAll({
      where: {
        ...
        send_list_coaches: false
      },
      include: [{model: Instructor}]
    });

    console.log(`Enviando ${classReminder.length} recordatorios ...`);
