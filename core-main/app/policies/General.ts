...
import { default as auth } from '../controllers/v1/AuthController';
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (decoded) {
        ...
        try {
          req.session.user = await User.findByPk(decoded.id, { include: [Profile] }) as User;
        } catch (err) {
          return Controller.serverError(res, err);
        }

        if (req.session.user.password_change_date && decoded.exp <= req.session.user.password_change_date) {
          throw new Error('Password changed after token was generated');
        }
