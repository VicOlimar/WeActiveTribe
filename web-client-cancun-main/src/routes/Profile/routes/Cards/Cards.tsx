...
import WithPaymentGateway, { PaymentGatewayProps } from '../../../../shared/WithPaymentGateway/WithPaymentGateway';
...
class Cards extends Component<Props, State> {
  ...
  /**
   * Get the user payment gateway cards from backend
   */
  getUserCards = async () => {
   ...
   */
  handleOnSubmitModal = async (values: any, gatewayResponse: PaymentGatewayTokenizeResponse | any) => {
    const card: Card | undefined = await CardsService.create(gatewayResponse.id);
    if (card) {
...
export default WithPayPal(WithPaymentGateway(Cards));